...
class PlaceManager{
    ...
    async changeAPIServices(uid, coordinates, services){
      return await this.database.collection("users").doc(uid).get()
      .then(async (documentSnapshot)=>{
        let data = documentSnapshot.data();
        if (data){
          ...
          for(let i = 0;i<services.length;i++){
            if (services[i]!=null){
              data.places[key].services[i]=services[i];
            } 
          }
          let result = await this.database.collection("users").doc(uid).set(data);
          return "Success";
        ...
        console.log("Error changing API services fot that users' place.");
        throw exceptionManager(error);
      });
    }
    ...
    async changeAllAPIServices(uid, services){
      ...
      .then(async (documentSnapshot)=>{
        let data = documentSnapshot.data();
        if (data){
          if (Object.keys(data.places).length===0) throw 'This user has no places registered.';
          for(let i = 0;i<services.length;i++){
            if (services[i]!=null){
        ...
        console.log("Error changing API services fot the places of that user.");
        throw exceptionManager(error);
      });
    }
